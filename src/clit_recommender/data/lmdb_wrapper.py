...
from clit_recommender.util import batch_items
...
class LmdbImmutableDict(Mapping[K, V]):
        ...
        with self.env.begin(write=True) as txn:
            for key, value in zip(keys, values):
                try:
                    ...
                    logging.debug("skipping %s, error: %s", key, err)
    ...
    @classmethod
    def from_dict(
        cls, input_dict: Dict[str, Any], output_file_path: str
    ) -> "LmdbImmutableDict[Any, Any]":
        if os.path.exists(output_file_path):
            print(f"Skipping conversion as {output_file_path} already exists.")
        else:
            output_lmdb_dict = LmdbImmutableDict(output_file_path, write_mode=True)
            for batch in tqdm(
                list(batch_items(input_dict.items(), n=250000)),
